;((wp) => {
  ...
  // ---------- draggable global launcher ----------
  const QA_GLOBAL_LAUNCHER_POS_KEY = 'qa_global_launcher_xy_v2';

  const QA_GLOBAL_LEGACY_DRAG_KEYS = [
    'qa_global_fab_pos',
    'qa_global_launcher_pos',
    'qa_global_widget_pos',
    'qa_global_icon_only_pos',
    'qa_global_icon_only_pos_fixed',
    'qa_global_icon_only_pos_transform',
    'qa_global_launcher_xy',
    'qa_gl_global_fab_pos',
    'qa_gl_global_launcher_pos',
    'qa_gl_global_widget_pos',
    'qa_gl_global_icon_only_pos',
    'qa_gl_global_icon_only_pos_transform'
  ];

  function qaClamp(n, min, max) {
    return Math.max(min, Math.min(max, n));
  }

  function qaClearLegacyGlobalDragKeys() {
    try {
      QA_GLOBAL_LEGACY_DRAG_KEYS.forEach((k) => localStorage.removeItem(k));
    } catch (_) {}
  }

  function qaReadLauncherPos() {
    try {
      const raw = localStorage.getItem(QA_GLOBAL_LAUNCHER_POS_KEY);
      if (!raw) return null;
  ...
  function qaSaveLauncherPos(x, y) {
    try {
      localStorage.setItem(QA_GLOBAL_LAUNCHER_POS_KEY, JSON.stringify({ x, y }));
    } catch (_) {}
  }

  function qaGetDirectFab(wrapper) {
    if (!wrapper || !wrapper.children) return null;
    for (let i = 0; i < wrapper.children.length; i++) {
      const child = wrapper.children[i];
      if (child && child.classList && child.classList.contains('qa-fab')) return child;
    }
    return wrapper.querySelector ? wrapper.querySelector('.qa-fab') : null;
  }

  function qaNormalizeFabForWrapperDrag(fab) {
    if (!fab || !fab.style) return;

    // Very old builds wrote viewport coordinates directly to the button.
    // For the new drag model, only the wrapper owns viewport position.
    [
      'inset', 'left', 'top', 'right', 'bottom',
      'transform', 'translate', 'margin-left', 'margin-top'
    ].forEach((prop) => {
      try { fab.style.removeProperty(prop); } catch (_) {}
    });
    ...
    fab.style.setProperty('-webkit-user-drag', 'none', 'important');
  }

  function qaGetLauncherSize(wrapper) {
    const fab = qaGetDirectFab(wrapper);
    const rect = (fab || wrapper).getBoundingClientRect();
    return {
      width: rect.width || (fab && fab.offsetWidth) || wrapper.offsetWidth || 82,
      height: rect.height || (fab && fab.offsetHeight) || wrapper.offsetHeight || 82
    };
  }

  function qaClampLauncherXY(x, y, wrapper) {
    const size = qaGetLauncherSize(wrapper);
    const pad = 8;
    return {
      x: qaClamp(x, pad, Math.max(pad, window.innerWidth - size.width - pad)),
      y: qaClamp(y, pad, Math.max(pad, window.innerHeight - size.height - pad))
    };
  }

  function qaApplyLauncherXY(wrapper, x, y, shouldSave) {
    if (!wrapper || !wrapper.style) return { x: 0, y: 0 };

    const fab = qaGetDirectFab(wrapper);
    qaNormalizeFabForWrapperDrag(fab);
    ...
    wrapper.style.setProperty('will-change', 'left, top', 'important');

    const clamped = qaClampLauncherXY(x, y, wrapper);
    wrapper.style.setProperty('left', clamped.x + 'px', 'important');
    wrapper.style.setProperty('top', clamped.y + 'px', 'important');

    if (shouldSave) qaSaveLauncherPos(clamped.x, clamped.y);
    return clamped;
  }

  function qaApplyStoredOrDefaultLauncherPos(wrapper) {
    if (!wrapper) return { x: 0, y: 0 };

    const saved = qaReadLauncherPos();
    if (saved) return qaApplyLauncherXY(wrapper, saved.x, saved.y, false);

    // Preserve the natural bottom-right placement on first load, then convert it
    // to explicit left/top coordinates only in the browser layer.
    const rect = wrapper.getBoundingClientRect();
    const fallbackX = Number.isFinite(rect.left) && rect.width ? rect.left : (window.innerWidth - 102);
    const fallbackY = Number.isFinite(rect.top) && rect.height ? rect.top : (window.innerHeight - 168);
    return qaApplyLauncherXY(wrapper, fallbackX, fallbackY, false);
  }

  function qaInstallGlobalLauncherDrag() {
    if (window.__qaGlobalLauncherDragV2Installed) return;
    window.__qaGlobalLauncherDragV2Installed = true;

    qaClearLegacyGlobalDragKeys();

    let suppressClickUntil = 0;
    let active = null;

    function applyAll() {
      document.querySelectorAll('.qa-floating').forEach((wrapper) => {
        qaApplyStoredOrDefaultLauncherPos(wrapper);
      });
    }
    ...
    document.addEventListener('click', function(e) {
      ...
      const hitFab = e.target && e.target.closest && e.target.closest('.qa-floating > .qa-fab');
      if (!hitFab) return;
      e.preventDefault();
      e.stopPropagation();
    ...
    document.addEventListener('pointerdown', function(e) {
      ...
      const fab = e.target.closest('.qa-floating > .qa-fab');
      if (!fab) return;
      if (e.button !== undefined && e.button !== 0) return;

      const wrapper = fab.closest('.qa-floating');
      if (!wrapper) return;

      const current = qaApplyStoredOrDefaultLauncherPos(wrapper);
      const rect = wrapper.getBoundingClientRect();
      ...
      fab.classList.add('qa-is-dragging');
      wrapper.classList.add('qa-is-dragging');
    }, true);
    ...
    document.addEventListener('pointermove', function(e) {
      ...
      active.moved = true;
      const next = qaApplyLauncherXY(
        active.wrapper,
        e.clientX - active.grabX,
        e.clientY - active.grabY,
        true
      );
      active.x = next.x;
      active.y = next.y;
    ...
    function finish(e) {
      ...
      const fab = qaGetDirectFab(active.wrapper);
      if (fab) {
        try { fab.releasePointerCapture(active.pointerId); } catch (_) {}
        fab.classList.remove('qa-is-dragging');
      }
      if (active.wrapper) active.wrapper.classList.remove('qa-is-dragging');

      if (active.moved) {
        qaSaveLauncherPos(active.x, active.y);
  ...
  function GlobalWidget({ cfg }) {
    ...
    useEffect(() => {
      // Global Chat rule: drag the launcher wrapper by the button handle.
      // This avoids the old fixed/inset button coordinates and the small-step transform drift.
      qaInstallGlobalLauncherDrag();
    }, []);
